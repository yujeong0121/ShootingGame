...
class food:
    ...
    def load_car(self):
        # 이미지중에서 랜덤으로 선택한다.
        self.image = pygame.image.load(random.choice(self.car_image1))
        self.rect = self.image.get_rect()

        rockWidth = self.rect[0]
        rockHeight = self.rect[1]
        self.rect.x = random.randrange(0, padWidth - rockWidth)
        self.rect.y = 0
        self.dy = random.randint(2, 15)
    ...
    while not onGame:
        ...
        if gametypeNum == 0: # 최초 캐릭터 선택
         ...
         choiceCharacter() #캐릭터를 선택해주세요 글씨
            ...
            for i in range(3):
                foodlist[i].draw_car()
                foodlist[i].rect.y +=foodlist[i].dy
